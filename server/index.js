...
app.get('/api/check-status', async (req, res) => {
  ...
  try {
    ...
    const sentimentRes = await pool.query('SELECT created_at FROM sentiment_feedbacks WHERE ip = $1 ORDER BY created_at DESC LIMIT 1', [ip]);
    ...
    if (sentimentRow) {
      const lastTime = new Date(sentimentRow.created_at).getTime();
      const now = Date.now();
...
app.post('/api/feedback', async (req, res) => {
  ...
  try {
    if (type === 'sentiment') {
      const lastRes = await pool.query('SELECT created_at FROM sentiment_feedbacks WHERE ip = $1 ORDER BY created_at DESC LIMIT 1', [ip]);
